...
app.use((req, res, next) => {
  ...
  res.setHeader('X-Powered-By', 'Azaklar API Server');
  next();
});
...
// Rate limiting için basit middleware (production'da Redis ile geliştirilmeli)
...
app.use((req, res, next) => {
  ...
  if (!rateLimitMap.has(ip)) {
    rateLimitMap.set(ip, { count: 1, resetTime: now + windowTime });
    return next();
  }
  ...
  if (now > userData.resetTime) {
    userData.count = 1;
    userData.resetTime = now + windowTime;
    return next();
  }
...
app.get('/api/health', (req, res) => {
  const healthData = {
    status: 'OK',
    message: 'Azaklar İnşaat API Server is running',
...
app.get('/api', (req, res) => {
  res.json({
    ...
    endpoints: {
      health: '/api/health',
      kentselEmail: '/api/send-kentsel-email',
...
app.use('*', (req, res) => {
  res.status(404).json({
    ...
    availableEndpoints: ['/api', '/api/health', '/api/send-kentsel-email', '/api/send-contact-email']
  });
});
...
app.use((error, req, res, next) => {
  ...
  if (error.message === 'Not allowed by CORS') {
    return res.status(403).json({
      success: false,
      message: 'CORS hatası: Bu origin\'e izin verilmiyor'
    });
  }
...
const server = app.listen(PORT, () => {
  console.log(`🚀 Azaklar İnşaat API Server ${PORT} portunda çalışıyor`);
